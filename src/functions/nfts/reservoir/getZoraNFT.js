...
const ZORA_URL = "https://api-zora.reservoir.tools/users/";
const RESERVOIR_API_KEY = process.env.RESERVOIR_API_KEY;

const getZoraNFT = async (user_id, evm_address) => {
  let nfts = [];
  ...
  for (let i = 0; i < nfts.length; i++) {
    ...
    formattedNFTs.push({
      ...
      openseaLink: `https://opensea.io/assets/zora/${nfts[i].collection.id}/${nfts[i].tokenId}`,
  ...
  console.log(`Zora NFTs: ${formattedNFTs.length}`);

  return formattedNFTs;
};

module.exports = getZoraNFT;
