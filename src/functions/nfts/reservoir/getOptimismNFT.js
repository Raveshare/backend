...
const OPTIMISM_URL = "https://api-optimism.reservoir.tools/users/";
const RESERVOIR_API_KEY = process.env.RESERVOIR_API_KEY;

const getOptimismNFT = async (user_id, evm_address) => {
  console.log("Optimism NFTs API Calls end", new Date().toISOString());
  let nfts = [];
  ...
  for (let i = 0; i < nfts.length; i++) {
    ...
    formattedNFTs.push({
      ...
      openseaLink: `https://opensea.io/assets/optimism/${nfts[i].collection.id}/${nfts[i].tokenId}`,
  ...
  console.log(`Optimism NFTs: ${formattedNFTs.length}`);

  return formattedNFTs;
};

module.exports = getOptimismNFT;
